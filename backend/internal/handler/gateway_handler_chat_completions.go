	...
	for {
		...
		if err != nil {
			...
			if len(fs.FailedAccountIDs) == 0 {
				...
				cls := classifyDynamicNoAccountErrorFromGin(c, h.gatewayService, apiKey, reqModel, reqModel, groupPlatform, &dynamicNoAccounts)
		...
		} else if shouldUseQoderProxy(account) {
			if h.qoderGatewayService == nil {
				h.chatCompletionsErrorResponse(c, http.StatusBadGateway, "upstream_error", "Qoder proxy service is not configured")
				if accountReleaseFunc != nil {
					accountReleaseFunc()
				}
				return
