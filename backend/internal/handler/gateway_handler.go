...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		for {
			...
			if err != nil {
				if len(dynamicGroups) > 0 && currentAPIKey.Group != nil && currentAPIKey.Group.RetryWithinGroupOnFailover && len(fs.FailedAccountIDs) > 0 {
			...
			if err != nil {
				...
				if errors.As(err, &failoverErr) {
					...
					if len(dynamicGroups) > 0 && currentAPIKey.Group != nil && currentAPIKey.Group.RetryWithinGroupOnFailover {
						switch action := fs.HandleFailoverError(c.Request.Context(), h.gatewayService, account.ID, account.Platform, account.GetPoolModeRetryCount(), failoverErr); action {
						case FailoverContinue:
							continue
						case FailoverCanceled:
							failoverClientGone(c)
...
func (h *GatewayHandler) compositeAvailableModels(ctx context.Context, groupID *int64) []string {
	...
	for _, platform := range []string{service.PlatformAnthropic, service.PlatformGemini, service.PlatformOpenAI, service.PlatformAntigravity, service.PlatformGrok, service.PlatformCodeBuddy, service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax, service.PlatformOpenCodeGo} {
...
func defaultCodexModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformCodeBuddy:
		return []string{"auto", "glm-5.2", "deepseek-v4-pro", "kimi-k2.7"}
	default:
