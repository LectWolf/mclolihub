...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	require.Equal(t, 23, snapshot.Version, "v23 起认证快照同时携带动态路由平台范围和分组免费 Fast 开关")
	require.Equal(t, RoutePlatformOpenAI, snapshot.RoutePlatform)
	...
	require.NoError(t, err)
	require.True(t, used)
	require.Equal(t, RoutePlatformOpenAI, materialized.RoutePlatform)
