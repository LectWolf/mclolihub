...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	if shouldClearStickySession(account, requestedModel) || !account.IsOpenAI() || !account.IsSchedulableForRequest(ctx) {
		_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
		return 0, nil, "", nil
	}
	...
	if s.schedulerSnapshot != nil && s.accountRepo != nil {
		...
		if shouldClearStickySession(latest, requestedModel) || !latest.IsOpenAI() || !latest.IsSchedulableForRequest(ctx) {
			_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
			return 0, nil, "", nil
		}
