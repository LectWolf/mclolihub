...
--
-- Numbered 256 on this fork (upstream 238 collided with compaction + an
-- upstream duplicate 238). Runs after 255_add_codebuddy_platform.sql.
-- DROP ... IF EXISTS + 幂等守卫保证可重入；新约束是 255 的超集，必须同时保留 MiniMax 和 CodeBuddy。
    ...
    IF monitor_constraint_def IS NULL OR position('opencode_go' IN monitor_constraint_def) = 0 THEN
        ...
        ALTER TABLE channel_monitors
            ...
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'codebuddy', 'opencode_go'));
    END IF;
