-- Append kimi/zhipu/deepseek/minimax to channel_monitor_v2_config.platforms.
-- Factory seed (194/197) only listed the original six platforms, so CN
-- OpenAI-compatible providers never appeared in V2 settings or V3 query scope.
    ...
    platforms = (
        ...
        FROM (
            ...
            FROM jsonb_array_elements(
                '[{"platform":"kimi","enabled":true,"models":[]},{"platform":"zhipu","enabled":true,"models":[]},{"platform":"deepseek","enabled":true,"models":[]},{"platform":"minimax","enabled":true,"models":[]}]'::jsonb
            ) AS missing(elem)
            WHERE NOT EXISTS (
                SELECT 1
...
WHERE id = 1
  AND (
      ...
      OR NOT (platforms @> '[{"platform":"minimax"}]'::jsonb)
  );
