...
func TestOpenCodeGoPlatformMigration(t *testing.T) {
	content, err := FS.ReadFile("256_opencode_go_platform.sql")
	...
	require.Contains(t, sql,
		"CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok', 'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'codebuddy', 'opencode_go'))")
}
