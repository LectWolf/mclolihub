...
type APIKeyQuery struct {
	...
	withGroupPreferences *APIKeyGroupPreferenceQuery
...
// QueryGroupPreferences chains the current query on the "group_preferences" edge.
func (_q *APIKeyQuery) QueryGroupPreferences() *APIKeyGroupPreferenceQuery {
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withGroupPreferences: _q.withGroupPreferences.Clone(),
...
// WithGroupPreferences tells the query-builder to eager-load the nodes that are connected to
// the "group_preferences" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithGroupPreferences(opts ...func(*APIKeyGroupPreferenceQuery)) *APIKeyQuery {
	query := (&APIKeyGroupPreferenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupPreferences = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withGroupPreferences; query != nil {
		if err := _q.loadGroupPreferences(ctx, query, nodes,
			func(n *APIKey) { n.Edges.GroupPreferences = []*APIKeyGroupPreference{} },
			func(n *APIKey, e *APIKeyGroupPreference) {
				n.Edges.GroupPreferences = append(n.Edges.GroupPreferences, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadGroupPreferences(ctx context.Context, query *APIKeyGroupPreferenceQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIKeyGroupPreference)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikeygrouppreference.FieldAPIKeyID)
	}
	query.Where(predicate.APIKeyGroupPreference(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.GroupPreferencesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
