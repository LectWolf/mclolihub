<template>
  <div class="flex flex-col gap-0.5">
    ...
    <CapacityBadge
      ...
      :current="concurrencyDisplay.current"
      :max="concurrencyDisplay.max"
...
// A limit of 0 means "unlimited", and the gateway skips the Redis slot entirely
// for such accounts — nothing is counted, so the in-flight number is unknown
// rather than zero. Rendering it as 0/0 made an untracked account look idle.
const hasConcurrencyLimit = computed(() => props.account.concurrency > 0)

const concurrencyDisplay = computed(() => {
  if (!hasConcurrencyLimit.value) {
    return { current: '–', max: '∞' }
  }
  return { current: currentConcurrency.value, max: props.account.concurrency }
})

const concurrencyTooltip = computed(() =>
  hasConcurrencyLimit.value ? '' : t('admin.accounts.capacity.concurrency.untracked')
)

const concurrencyClass = computed(() => {
  if (!hasConcurrencyLimit.value) return 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'
