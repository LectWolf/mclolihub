    ...
    >
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <button
            type="button"
            data-testid="create-excel-bps-toggle"
...
// Qoder 使用自己的凭据表单（个人访问令牌或设备登录），不走通用的 API Key / Base URL 输入。
const hidesGenericApiKey = computed(() => form.platform === 'qoder')
...
const qoderDeviceLoginUser = computed(() =>
  qoderAccessToken.value
    ? qoderLoginEmail.value || qoderLoginName.value || qoderUserID.value || '-'
    : ''
)
const clearQoderDeviceLogin = () => {
  qoderAccessToken.value = ''
...
const resetQoderForm = () => {
  qoderPersonalToken.value = ''
  qoderMachineId.value = ''
  qoderRegion.value = 'cn'
  clearQoderDeviceLogin()
}
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (form.type === 'oauth' && excelBPSEnabled.value) {
...
const handleSubmit = async () => {
  ...
  // Qoder：个人访问令牌与设备登录二选一（都填时优先设备登录，个人令牌作为兜底）。
  if (form.platform === 'qoder') {
    if (!form.name.trim()) {
      appStore.showError(t('admin.accounts.pleaseEnterAccountName'))
      return
    ...
    if (access) {
      ...
      if (qoderLoginName.value.trim()) credentials.name = qoderLoginName.value.trim()
      if (qoderLoginEmail.value.trim()) credentials.email = qoderLoginEmail.value.trim()
    }
