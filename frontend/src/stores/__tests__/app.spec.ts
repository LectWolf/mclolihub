...
describe("useAppStore", () => {
  ...
  describe("双更新通道", () => {
    it("分别保存自定义版本和 Sub2API 上游版本", async () => {
      vi.mocked(checkUpdates).mockResolvedValue({
        current_version: "0.1.179.1",
        latest_version: "0.1.179.2",
        has_update: true,
        cached: false,
        build_type: "release",
        custom: {
        ...
        upstream: {
          ...
          has_update: true,
          release_info: {
            name: "Sub2API 0.1.180",
            body: "",
            published_at: "2026-08-21T00:00:00Z",
            html_url:
              "https://github.com/Wei-Shaw/sub2api/releases/tag/v0.1.180",
          },
        },
      });
      const store = useAppStore();

      await expect(store.fetchVersion(true)).resolves.not.toBeNull();

      expect(store.currentVersion).toBe("0.1.179.1");
      expect(store.latestVersion).toBe("0.1.179.2");
      expect(store.hasUpdate).toBe(true);
      expect(store.upstreamCurrentVersion).toBe("0.1.179");
      expect(store.upstreamLatestVersion).toBe("0.1.180");
      expect(store.upstreamHasUpdate).toBe(true);
      expect(store.upstreamReleaseInfo?.name).toBe("Sub2API 0.1.180");
    });
  });
  ...
  describe("公开设置加载", () => {
    ...
    it("fetchPublicSettings(force) 会同步更新运行时注入配置", async () => {
      ...
      expect((window as any).__APP_CONFIG__.table_page_size_options).toEqual([
        20, 100, 1000,
      ]);
