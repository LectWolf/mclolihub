...
export default {
  channelMonitorV3: {
    ...
    probingBadge: 'Live probe',
    probingHint: 'This group has scheduled probes; the latest probe fills status and first-token when traffic samples are insufficient',
    probeTtftHint: 'From the group probe, not this traffic window',
    timelineTooltip: '{time} · Availability {availability} · Cache {cache} · First token {ttft}',
    probeTimelineTooltip: 'Probe {time} · success {success} · failure {failure} · first token {ttft}',
    mixedTimelineTooltip: '{time} · Availability {availability} (includes probe success {success} / failure {failure}) · Cache {cache} · First token {ttft}',
