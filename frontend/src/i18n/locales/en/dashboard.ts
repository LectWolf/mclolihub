export default {
  ...
  keys: {
    ...
    naturalRevert: 'Natural return',
    naturalRevertFormula: 'After failover, cheapest-first uses at least 5 successful requests from the same user and model over the last 6 hours to compare the next 3 session requests (2 without a session): keeping the current warm-cache cost versus one cold return request plus later warm requests. When data is insufficient, a preferred group that is 65% cheaper returns immediately. Fastest-first still returns when the target is at least 3 seconds and 35% faster. Holds are capped at 3 minutes or 8 requests; failures, 429s, and unavailable groups switch immediately.',
    holdingGroup: 'Held group',
    holdingRoute: 'Holding {group} for cache locality ({model})',
    holdingCostEstimate: 'From {samples} successful same-model requests over 6h, projected for {requests} requests: hold ${keep}; return now ${return}.',
    returnNow: 'Return now',
    returnNowSuccess: 'Temporary route cleared; the next request will use the current policy',
    disabledGroups: 'Excluded groups',
    customGroupOrder: 'Custom group order',
    nextGroup: 'Next expected',
    routingPreviewTitle: 'Routing order preview',
    routingPreviewMode: 'Current strategy: {mode}',
    routingPreviewError: 'Failed to load routing order',
  ...
  groupHealth: {
    ...
    cacheRateOverall: 'Overall cache hit rate {value}',
    cacheRate6h: '6h cache hit rate {value}',
    cacheRateOverallShort: 'Overall {value}',
    trend: 'Last 12 hours',
