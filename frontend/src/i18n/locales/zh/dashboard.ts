export default {
  ...
  keys: {
    ...
    selectGroup: '选择分组',
    smartRouting: '智能路由',
    smartRoutingHint: '开启后按添加顺序尝试分组。协议不匹配、超过最大倍率或探测失败的分组会跳过，并按实际命中的分组计费。',
    smartRoutingAdd: '添加路由',
    smartRoutingEmpty: '请添加至少一个分组',
    smartRoutingMax: '最多添加 10 个分组',
    smartRoutingChangeInEditor: '点击可添加或调整智能路由顺序',
    routeModeLabel: '路由方式',
    routeModes: { fixed: '固定分组', smart: '智能路由' },
    ...
    smartRouteGroups: '智能路由分组',
    smartRouteHint: '打开开关纳入路由，拖拽调整优先级。超过最大倍率或探测失败的分组会自动跳过。',
    smartRouteRequired: '智能路由至少选择一个分组',
    nextGroup: '预计下一组',
