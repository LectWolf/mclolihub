export default {
  ...
  keys: {
    ...
    naturalRevert: '自然回切',
    naturalRevertFormula: '故障切换后短暂保持当前分组以保留缓存。低价优先在最近 6 小时同用户、同模型至少 5 次成功请求且存在缓存读取时，比较未来 3 次会话请求（无会话为 2 次）：保留成本 = 当前组热缓存成本 × 次数；回切成本 = 目标组首次冷缓存成本 + 后续热缓存成本。真实数据不足时，目标便宜 65% 以上立即回切。响应优先仍在目标快至少 3 秒且快 35% 以上时立即回切。最多保持 3 分钟或 8 次请求，分组故障、429 或不可用时立即切换。',
    holdingGroup: '暂留分组',
    holdingRoute: '为保留缓存，暂留 {group}（{model}）',
    holdingCostEstimate: '按近 6 小时同模型 {samples} 次成功请求，未来 {requests} 次预估：暂留 ${keep}，立即回切 ${return}。',
    returnNow: '立即回切',
    returnNowSuccess: '已清除暂留路由，下一次请求将按当前策略选择',
    disabledGroups: '禁用分组',
    customGroupOrder: '自定义分组顺序',
    nextGroup: '预计下一组',
    routingPreviewTitle: '路由顺序预览',
    routingPreviewMode: '当前策略：{mode}',
    routingPreviewError: '加载路由顺序失败',
    dragToReorder: '拖拽排序',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
  ...
  groupHealth: {
    title: '分组渠道状态',
    window: '最近 12 小时，每 10 分钟一格；成功且不超过 10 秒为绿色，超过 10 秒为黄色，不可用为红色，紫色为用户真实首字；柱高 30 秒封顶',
    group: '分组',
    multiplier: '倍率',
    status: '状态',
    realTtft: '真实首字延迟',
    ...
    cacheRate: '缓存命中率',
    cacheRateOverall: '总体缓存命中率 {value}',
    cacheRate6h: '6 小时缓存命中率 {value}',
    cacheRateOverallShort: '总体 {value}',
    trend: '最近 12 小时状态',
    empty: '暂无可查看的分组',
