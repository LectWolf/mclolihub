export default {
    ...
    dashboard: {
      ...
      todayOverview: '今日概览',
      deltaCaption: '涨跌幅为较昨日同时段',
      statsUpdatedAt: '数据生成于 {time}',
      statsStale: '统计延迟',
      statsStaleHint: '预聚合统计最后更新于 {time}，今日数据可能滞后',
      yesterdaySamePeriod: '昨日同时段 {value}',
      noYesterdayData: '昨日同时段无可比数据',
      todaySpend: '今日消费',
      todaySpendHint: '用户实际扣费合计（已计入分组倍率与折扣），即平台收入。',
      todayProfit: '今日毛利',
      todayProfitHint: '实际扣费 − 账号成本。账号成本按账号计费倍率（或账号统计定价）折算。',
      standardCostValue: '标准 {value}',
      accountCostValue: '成本 {value}',
      marginValue: '毛利率 {value}',
      marginLabel: '毛利率',
      cumulativeValue: '累计 {value}',
      cacheHitValue: '缓存命中 {value}',
      tokenBreakdown: '输入 {input} · 输出 {output} · 缓存写入 {cacheWrite} · 缓存读取 {cacheRead}',
      accountPool: '账号池',
      accountPoolHint: '按账号当前状态互斥分类，与账号列表的状态筛选口径一致，点击分类可直接跳转筛选。',
      schedulableOfTotal: '可调度 / 账号总数',
      activeOfTotal: '正常 / 账号总数',
      errorAccountsFallback: '{n} 个账号异常',
      overloadedHint: '其中 {n} 个上游过载中',
      accountHealth: {
        available: '可调度',
        rateLimited: '限流中',
      ...
      usersCard: '用户与密钥',
      newUsersValue: '今日 +{n}',
      activeUsersValue: '今日活跃 {n}',
      hourlyActiveUsersValue: '本小时 {n}',
      apiKeysValue: 'API 密钥 {total} · 启用 {active}',
      realtimeLoad: '实时负载',
      realtimeLoadHint: 'RPM / TPM 为近 5 分钟的每分钟平均值，TPM 只统计输入与输出 Token。',
      todayAvgResponseValue: '今日平均响应 {value}',
      uptimeValue: '服务已运行 {value}',
      uptimeUnits: {
        day: '天',
        hour: '小时',
        minute: '分钟'
      ...
      analysisTitle: '数据分析',
      spendTrend: '消费与成本趋势',
      spendTrendHint: '实际消费为用户扣费，账号成本为上游账号折算成本，毛利 = 实际消费 − 账号成本。',
      seriesActualCost: '实际消费',
      seriesAccountCost: '账号成本',
      seriesProfit: '毛利'
    },
    ...
    groups: {
      ...
      refreshHealth: '立即探测',
      refreshHealthSuccess: '分组探测已完成',
      refreshHealthError: '分组探测失败',
      sortOrder: '排序',
      columnSettings: '列设置',
